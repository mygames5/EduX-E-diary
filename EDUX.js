/* EDUX JS — весь интерактив, каждый пиксель анимирован */
    ...
    document.querySelectorAll('.edux_news-card, .edux_review-card').forEach(card => {
        card.addEventListener('mousemove', (e) => {
            ...
            const glare = card.querySelector('.edux_card-glare');
            if (glare) {
    ...
    document.querySelectorAll(
        '.edux_news-card, .edux_review-card, .edux_section-header, .edux_glass-card, .edux_footer-contact-item, .edux_hero-stat, .edux_hero-badge, .edux_hero-subtitle'
    ).forEach(el => observer.observe(el));

    //  3. ripple эффект на кнопках,чисто  
    document.querySelectorAll('.edux_btn').forEach(btn => {
    ...
    const heroImg = document.querySelector('.edux_hero-image');
    if (heroImg) {
    ...
    const statObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                const numEl = entry.target.querySelector('.edux_stat-num');
    ...
    document.querySelectorAll('.edux_hero-stat').forEach(s => statObserver.observe(s));

    // 6. активный nav-линк при скролле
    // подсвечивает текущую секцию в меню
    const sections = document.querySelectorAll('section[id], footer[id]');
    const navLinks = document.querySelectorAll('.edux_nav-link');
    const navObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                navLinks.forEach(link => {
                    link.classList.remove('edux_nav-link--active');
                    if (link.getAttribute('href') === '#' + entry.target.id) {
                        link.classList.add('edux_nav-link--active');
                    }
                });
            }
        });
    ...
    document.querySelectorAll('.edux_faq-item').forEach(item => {
        item.addEventListener('toggle', function() {
            if (this.open) {
                document.querySelectorAll('.edux_faq-item[open]').forEach(other => {
